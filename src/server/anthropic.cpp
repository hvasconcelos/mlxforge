...
void append_message(const json& m, std::vector<Tokenizer::Message>& out,
  ...
  // base64 image blocks are decoded into images_out, in order.
  std::string text;
  ...
  for (const auto& b : *it) {
    ...
    if (type == "text") {
      ...
      if (src != b.end() && src->is_object() && src->value("type", std::string()) == "base64")
        images_out.push_back(base64_decode(src->value("data", std::string())));
    }
  }
...
ChatRequest parse_messages_request(const nlohmann::json& body) {
  ...
  for (const auto& m : *it) append_message(m, r.messages, r.images);
